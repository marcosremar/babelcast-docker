...
import { startMeeting, isMeetingActive } from './meeting-runner'
import { SoundContext, VideoContext } from './media_context'
import { GLOBAL } from './singleton'
import { MeetingStateMachine } from './state-machine/machine'
import { MeetingEndReason } from './state-machine/types'
import { MeetingParams, StopRecordParams } from './types'
...
export async function server() {
    ...
    // Health check / version endpoint (responds immediately, no dependencies)
    app.get('/version', async (_req, res) => {
        console.log(`version requested`)
        const status = isMeetingActive() ? 'meeting_active' : 'idle'
        try {
            const buildInfo = await import('./buildInfo.json')
            res.status(200).json({ ...buildInfo, status })
        } catch {
            res.status(200).json({ status, buildDate: 'unknown' })
        }
    })

    // Join a meeting — accepts MeetingParams via POST body
    app.post('/join', async (req, res) => {
        if (isMeetingActive()) {
            return res.status(409).json({ error: 'A meeting is already active' })
        }

        const params = req.body as MeetingParams
        if (!params.meeting_url) {
            return res.status(400).json({ error: 'meeting_url is required' })
        }

        // Fill in defaults for fields the gateway may not provide
        if (!params.bot_uuid) params.bot_uuid = `bot-${Date.now()}`
        if (!params.bot_name) params.bot_name = 'BabelCast Bot'
        if (!params.recording_mode) params.recording_mode = 'speaker_view'
        // Ensure remote is null (not undefined) so isServerless() works correctly
        if (params.remote === undefined) params.remote = null
        if (!params.automatic_leave) {
            params.automatic_leave = {
        ...
        console.log(`[join] Starting meeting: ${params.meeting_url}`)

        // Start meeting in background (don't block HTTP response)
        startMeeting(params).catch((err) => {
